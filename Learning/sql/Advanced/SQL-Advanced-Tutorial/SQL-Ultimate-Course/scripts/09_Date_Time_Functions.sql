...
SELECT
    ...
    CreationTime,
    -- DATETRUNC Examples
    DATEFROMPARTS(year, CreationTime) AS Year_dt,
    DATEFROMPARTS(day, CreationTime) AS Day_dt,
    DATEFROMPARTS(minute, CreationTime) AS Minute_dt,
    -- DATENAME Examples
...
*/
SELECT
    DATEFROMPARTS(year, CreationTime) AS Creation,
    COUNT(*) AS OrderCount
FROM Sales.Orders
GROUP BY DATEFROMPARTS(year, CreationTime);
